...
In DBSCAN, observations are considered as locations in multidimensional space. The algorithm works by defining a cluster as a dense region of connected points. During the fitting process, points are classified as core points, border points, or noise based on their proximity to other points (**radius**) and a density threshold (**min_pts**).
...
1. For each observation in the data, determine whether each point is a core point. A core point is defined as an observation which has more than the specified **minpts** points (including the point itself) within the specified **radius** around the point.
