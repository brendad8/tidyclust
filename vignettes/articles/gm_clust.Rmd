...
To specify a GMM model in `tidyclust`, simply set the value for `num_clusters`:
...
**Should probably make some way to extract probabilities of belonging to each group**

```{r}
gm_clust_fit$fit$z %>% head()
```
...
```{r}
gmm_engine <- gm_clust_fit %>% extract_fit_engine()

mclust::plot.Mclust(gmm_engine)
```
